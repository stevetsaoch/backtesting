...
class OrderValidator(ABC, Generic[PG]):
    def __init__(
        self,
        ...
        self._result: dict[InstrumentId, dict[str, bool]] = defaultdict(dict)

    @property
    @abstractmethod
    def result(self) -> dict[InstrumentId, dict[str, bool]]: ...

    @abstractmethod
    def _validate_trading_session(self, instrument_id: InstrumentId) -> None: ...

    @abstractmethod
    def _validate_kill_switch(self, instrument_id: InstrumentId) -> None: ...

    @abstractmethod
    def _validate_available_chance(self, instrument_id: InstrumentId) -> None: ...
...
class ORBLongOrderValidator(OrderValidator[ActorInfoProvider]):
    ...
    def _validate_trading_session(self, instrument_id: InstrumentId):
        if (
            ...
            self._result[instrument_id]["trading_session_validation"] = False
            return
        elif (
            self._provider.get_current_datetime().time()
            ...
            self._result[instrument_id]["trading_session_validation"] = True
            return
    ...
    def _validate_kill_switch(self, instrument_id: InstrumentId):
        if (
            ...
            self._result[instrument_id]["kill_switch_validation"] = False
            return
        else:
            self._result[instrument_id]["kill_switch_validation"] = True
            return
    ...
    def _validate_available_chance(self, instrument_id: InstrumentId):
        ...
        if occupied_chance >= self._trading_rule.position_rule.open_position_maximum:
            self._result[instrument_id]["available_chance_validation"] = False
        else:
            self._result[instrument_id]["available_chance_validation"] = True
    ...
    def _validate_instrument_id_not_present_in_open_orders(
        ...
        if len(open_orders) > 0:
            self._result[instrument_id][
                "instrument_id_not_present_in_open_order_validation"
            ] = False
        else:
            self._result[instrument_id][
                "instrument_id_not_present_in_open_order_validation"
    ...
    def _validate_instrument_id_not_present_in_open_positions(
        ...
        if len(open_positions) > 0:
            self._result[instrument_id][
                "instrument_id_not_present_in_open_position_validation"
            ] = False
        else:
            self._result[instrument_id][
                "instrument_id_not_present_in_open_position_validation"
    ...
    def _validate_intraday_profit_and_loss(self, instrument_id: InstrumentId):
        ...
        if c_pnl >= self._trading_rule.risk_rule.intraday_loss_maximum:
            self._result[instrument_id]["intraday_profit_and_loss_validation"] = False
        elif c_pnl < self._trading_rule.risk_rule.intraday_loss_maximum:
            self._result[instrument_id]["intraday_profit_and_loss_validation"] = True
    ...
    def _validate_cost_minimum_and_risk_value_minimum(self, order_ticket: OrderTicket):
        ...
        if order_risk_value < opportunity_cost_minimum:
            self._result[order_ticket.instrument_id][
                "cost_minimum_and_risk_value_minimum_validation"
            ] = False
